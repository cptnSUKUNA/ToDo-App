...
function saveToDos() {
	localStorage.setItem(apiKey, JSON.stringify(allToDos));
}

function getAllToDos() {
	showLoading();
	allToDos = JSON.parse(localStorage.getItem(apiKey)) || [];
	displayData();
	hideLoading();
}
...
function deleteTodo(idTodo) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			showLoading();
			allToDos = allToDos.filter((todo) => todo._id !== idTodo);
			saveToDos();
			getAllToDos();
...
function markComplete(idTodo) {
	const todo = allToDos.find((todo) => todo._id === idTodo);
	if (todo) {
		todo.completed = true;
		saveToDos();
		getAllToDos();
...
function cancelComplete(idTodo) {
	const todo = allToDos.find((todo) => todo._id === idTodo);
	if (todo) {
...
function changeProgress() {
	...
	progressBar.style.width = totalTask > 0 ? `${(completedTaskNumber / totalTask) * 100}%` : "0%";
